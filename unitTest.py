...
if __name__ == "__main__":
    ...
    parser.add_option("-t", "--testcases", dest="tcs", help="Comma separated list of test cases to "
                                                            "run from the test file.", default="")
    parser.add_option("-x", "--exclude", dest="exs", help="Comma separated list of test cases to exclude from the "
                                                          "test file. Takes precedence over cases to run.", default="")
    (options, args) = parser.parse_args()

    if not options.testFile:
        print("ERROR: Test case not specified\nUsage: "+os.path.basename(__file__)+" [-v][-t <test_cases_to_run>] "
                                                                                   "[-x <test_cases_to_exclude>] "
                                                                                   "-f <test_file> -c <command_to_run> "
                                                                                   "<test_name>")
        sys.exit(0)
    ...
    if not os.path.exists(options.testFile):
        print("ERROR: Could not find ", options.testFile)
        sys.exit(0)

    if not os.path.exists(options.command):
        print("ERROR: Could not find ", options.command)
        sys.exit(0)
    testCases = []
    excludes = []
    if options.tcs:
    ...
    thisTest.test_file(options.testFile)

    if thisTest.ready:
        thisTest.execute(testCases, excludes)
    else:
        print("Test is not ready to execute.\n"+thisTest.error)
